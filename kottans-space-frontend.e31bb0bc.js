...
function renderApp() {
  ...
  missionRadios.forEach(radio => radio.addEventListener('change', ({
    target
...
function Rockets() {
  return `<div>
    ...
    rocket_name,
    id
  }) => `<label>${rocket_name}<input class="rocket-radio" type="radio" value="${rocket_name}" name="missions" ${rocket_name === window.dataStore.selectedRocket ? 'checked' : ''}/></label>`)}
        </div>
...
function Missions() {
  ...
  return `<div>
        ...
        ${missions.map(({
    mission_name
  }) => `<label>${mission_name}<input class="mission-radio" type="radio" value="${mission_name}" name="rockets" ${mission_name === window.dataStore.selectedMission ? 'checked' : ''}/></label>`)}
        </div>
